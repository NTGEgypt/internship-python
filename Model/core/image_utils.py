...
def normalize_image(image_bgr: np.ndarray, max_dim: int = config.PROCESSING_MAX_DIM):
    ...
    transform = NormalizationTransform(
        ...
        processing_width=new_w, processing_height=new_h,
        scale=scale, pad_x=0, pad_y=0,
    )
    return resized, transform, original
...
def assess_image_quality(image_bgr: np.ndarray) -> dict:
    ...
    return {
        ...
        "status": "good" if len(issues) == 0 else ("acceptable" if len(issues) == 1 else "poor"),
    }
