...
def validate_nid(raw_or_normalized_digits: str) -> NIDValidationResult:
    ...
    if not normalized or not normalized.isdigit():
        ...
        result.errors.append(f"Expected digits, got {raw_or_normalized_digits!r}.")
        return result

    # Best-effort structural extraction for available digits
    if len(normalized) >= 7:
    ...
    if len(normalized) >= 9:
        ...
        if gov_code in config.GOVERNORATE_CODES:
    ...
    # Strict full 14-digit validation
    if len(normalized) != 14:
        result.status = INVALID_FORMAT
        result.errors.append(f"Expected 14 digits, got {len(normalized)} characters ({normalized}).")
        return result
    ...
    if not result.derived_birth_date:
        result.status = INVALID_DATE
        result.errors.append(f"Invalid embedded date ({yy}-{mm}-{dd}).")
        return result

    if not result.derived_governorate_code:
